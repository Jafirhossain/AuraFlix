...
function getManifest(config) {
    const allCatalogs = [
        ...
        { type: "anime", id: "anime_movies", name: "🎬 Anime Movies", extra: [{ name: "search" }, { name: "skip" }] },
    ...
    return {
        id: "org.auraflix.master.engine",
        version: "15.0.0",
        name: "AuraFlix Ultra 🇮🇳",
        description: "Professional Torrentio & MediaFusion Class Engine. 100% Fixed OTT Hubs, TorBox/Debrid Integration, and Aggressive Hindi Streaming.",
...
async function fetchOTTContent(type, genre = null, search = null, skip = 0) {
    try {
        ...
        if (search) {
            ...
            url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_original_language=te|ta|kn|ml&sort_by=primary_release_date.desc&page=${page}`;
            ...
            url = `https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&with_networks=3106|159|1086|3186&sort_by=popularity.desc&page=${page}`;
            ...
            url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_original_language=en&sort_by=popularity.desc&vote_count.gte=100&page=${page}`;
        }
...
// ----------------------------------------------------
// 4. BULLETPROOF ROUTING MIDDLEWARE
// ----------------------------------------------------
const app = express();
...
// The Ultimate Stremio URL Parser (Solves all 404 / Empty Content bugs)
function parseStremioRequest(req) {
    let configStr = null, type, id, extraStr;
    let parts = req.path.split('/').filter(p => p);
    if (parts.length === 0) return {};
    
    if (parts[0] !== 'catalog' && parts[0] !== 'meta' && parts[0] !== 'stream') {
        configStr = parts.shift();
    }
    const routeType = parts.shift();
    type = parts.shift();
    id = parts.shift();
    extraStr = parts.shift();

    if (id) id = decodeURIComponent(id.replace('.json', ''));
    if (extraStr) extraStr = decodeURIComponent(extraStr.replace('.json', ''));
    ...
    return { configStr, routeType, type, id, extra };
}

app.get("/*", async (req, res, next) => {
    if (req.path === "/" || req.path.startsWith("/configure") || req.path.includes("manifest.json")) {
        return next();
    }
    
    const parsed = parseStremioRequest(req);
    if (!parsed.routeType) return next();

    const config = parseConfig(parsed.configStr);

    try {
        // --- CATALOG ROUTE ---
        if (parsed.routeType === "catalog") {
            const search = parsed.extra.search || null;
            const genre = parsed.extra.genre || null;
            const skip = parseInt(parsed.extra.skip) || 0;
            let metas = [];
            if (parsed.id.startsWith("anime")) metas = await fetchAnime(parsed.id, search, genre, skip);
            else metas = await fetchOTTContent(parsed.id, genre, search, skip);
            return res.json({ metas });
        }
        
        // --- META ROUTE (FIXED TV EPISODES) ---
        if (parsed.routeType === "meta") {
            if (parsed.id.startsWith("kitsu:")) {
                const cleanId = parsed.id.replace("kitsu:", "");
                ...
                return res.json({ meta: { id: parsed.id, type: isMovie ? "movie" : "anime", name: attr.canonicalTitle || attr.titles?.en, poster: attr.posterImage?.large, background: attr.coverImage?.large, description: attr.synopsis, videos } });
            } 
            else if (parsed.id.startsWith("tmdb:")) {
                const tmdbId = parsed.id.replace("tmdb:", "");
                const isTv = parsed.type === "series";
                ...
                const imdbId = m.external_ids?.imdb_id || m.imdb_id || parsed.id;
                const videos = [];
                ...
                return res.json({ meta: { id: parsed.id, type: parsed.type, name: m.title || m.name, poster: m.poster_path ? `https://image.tmdb.org/t/p/w500${m.poster_path}` : undefined, background: m.backdrop_path ? `https://image.tmdb.org/t/p/original${m.backdrop_path}` : undefined, description: m.overview, imdb_id: imdbId, videos: videos.length > 0 ? videos : undefined } });
            }
        }

        // --- STREAM ROUTE (MEDIAFUSION + TORRENTIO DUAL ENGINE) ---
        if (parsed.routeType === "stream") {
            let targetId = parsed.id;
            let isAnime = targetId.startsWith("kitsu:") || parsed.type === "anime";
            let mediaTitle = "";
            let seasonNum = null;
            let episodeNum = null;

            // Resolve proper IDs for Trackers
            if (targetId.startsWith("kitsu:")) {
                episodeNum = targetId.split(":")[2] || "1";
            } else if (targetId.startsWith("tmdb:")) {
                const parts = targetId.split(":");
                const tmdbId = parts[1];
                seasonNum = parts[2] || null;
                episodeNum = parts[3] || null;
                const isTv = parsed.type === "series" || Boolean(seasonNum);

                const tRes = await axios.get(`https://api.themoviedb.org/3/${isTv ? 'tv' : 'movie'}/${tmdbId}?api_key=${TMDB_API_KEY}&append_to_response=external_ids`, { timeout: 4000 }).catch(()=>null);
                if (tRes && tRes.data) {
            ...
            // 1. Torrentio Fetcher
            let torrentioUrl = `https://torrentio.strem.fun/stream/${parsed.type}/${targetId}.json`;
            if (config.debridProvider !== "none" && config.debridProvider !== "torbox" && config.debridToken) {
                torrentioUrl = `https://torrentio.strem.fun/${config.debridProvider}=${config.debridToken}/stream/${parsed.type}/${targetId}.json`;
            }
            fetchPromises.push(axios.get(torrentioUrl, { timeout: 7000 }).catch(()=>null));

            // 2. MediaFusion Fetcher (Crucial for TorBox, Indian Content, Hindi Hubs)
            let mfType = isAnime ? "anime" : parsed.type;
            let mfUrl = `https://mediafusion.fun/stream/${mfType}/${targetId}.json`;
            if (config.debridProvider === 'torbox' && config.debridToken) {
                const mfConfig = btoa(JSON.stringify({ streamingProviders: { torbox: { token: config.debridToken } } }));
                mfUrl = `https://mediafusion.fun/${mfConfig}/stream/${mfType}/${targetId}.json`;
            } else if (config.debridProvider === 'realdebrid' && config.debridToken) {
                const mfConfig = btoa(JSON.stringify({ streamingProviders: { realdebrid: { token: config.debridToken } } }));
                mfUrl = `https://mediafusion.fun/${mfConfig}/stream/${mfType}/${targetId}.json`;
            }
            fetchPromises.push(axios.get(mfUrl, { timeout: 7000 }).catch(()=>null));

            // Execute Dual Engine
            const responses = await Promise.all(fetchPromises);
            responses.forEach(res => {
                if (res && res.data && res.data.streams) {
                    allStreams = allStreams.concat(res.data.streams);
                }
            });
            ...
            allStreams.forEach(s => {
                ...
                let modeTag = s.url ? "⚡ DEBRID/DIRECT" : "🚀 HIGH-SPEED P2P";
                let cleanTitle = s.title ? s.title.split('\n')[0].replace(/\b(Torrentio|Debrid|MediaFusion)\b/ig, 'AuraFlix') : 'Instant Play';
        ...
        console.error(e);
        return res.json({ metas: [], streams: [], meta: {} });
    }
});

// ----------------------------------------------------
// 5. PROFESSIONAL UI HTML DASHBOARD (Torrentio/Comet Style)
// ----------------------------------------------------
function renderConfigPage(res, currentConfig) {
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #0f1115; color: #e2e8f0; margin: 0; padding: 20px 10px; }
                ...
                h1 { color: #f8fafc; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px; }
                ...
                .card-check { background: #1e293b; padding: 12px; border-radius: 8px; border: 1px solid #334155; display: flex; align-items: center; cursor: pointer; user-select: none; transition: border-color 0.2s, background 0.2s; font-size: 13px; color: #f8fafc; }
                ...
                select, input[type="text"] { width: 100%; padding: 12px; background: #1e293b; color: #f8fafc; border: 1px solid #334155; border-radius: 8px; font-size: 14px; box-sizing: border-box; outline: none; transition: border-color 0.2s; appearance: none; }
                ...
                .lang-item { display: flex; align-items: center; padding: 10px; border-radius: 6px; font-size: 13px; cursor: pointer; color: #f8fafc; transition: background 0.2s; }
                ...
                .btn { display: block; width: 100%; background: #4f46e5; color: white; padding: 15px; text-align: center; text-decoration: none; font-size: 15px; font-weight: 700; border-radius: 8px; margin-top: 30px; transition: background 0.2s; border: none; cursor: pointer; box-sizing: border-box; }
                .btn:hover { background: #4338ca; }
                .btn-copy { background: transparent; border: none; margin-top: 15px; font-size: 13px; color: #818cf8; text-decoration: underline; }
        ...
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <p class="desc">Provides Multi-OTT streams from Torrentio & MediaFusion. Highly optimized for TorBox, Debrid, and Free High-Speed P2P.</p>
                </div>
                ...
                <div class="grid-3">
                    ...
                    <label class="card-check"><input type="checkbox" id="ex_hdr" value="hdr"> HDR/Dolby Vision</label>
                    ...
                    <label class="card-check"><input type="checkbox" id="ex_480p" value="480p"> 480p</label>
                    <label class="card-check"><input type="checkbox" id="ex_cam" value="cam"> Screener / Cam</label>
                </div>
                ...
                <select id="sorting">
                    ...
                    <option value="seeders_first">Highest seeders & file size first</option>
                </select>
                ...
                <div class="debrid-container">
                    <select id="debridProvider" onchange="toggleDebridInput()">
                        <option value="none">None (Free P2P)</option>
                        <option value="torbox">TorBox (MediaFusion Integration)</option>
                ...
                <a id="installBtn" class="btn" href="#">INSTALL</a>
                <div style="text-align: center;">
                    <button id="copyBtn" class="btn-copy" onclick="copyManifestLink()">Copy Link</button>
                </div>
            </div>
